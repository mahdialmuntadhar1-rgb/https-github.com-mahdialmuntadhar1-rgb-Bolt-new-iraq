...
const GRADIENT_FALLBACKS = [
  'linear-gradient(135deg, #1a1040 0%, #0d2137 50%, #0a1628 100%)',
  'linear-gradient(135deg, #0f1f3d 0%, #1a3a2a 50%, #0d1e1a 100%)',
  'linear-gradient(135deg, #1e0a3d 0%, #0d1e3d 50%, #0a2020 100%)',
];
        ...
        <section className="relative h-[60vh] md:h-screen w-full overflow-hidden flex flex-col justify-center items-center">
            ...
            >
                {heroSlides.map((slide, idx) => (
                    <div
                        key={slide.id}
                        className="relative w-full h-full flex-shrink-0 snap-start flex items-center justify-center text-center"
                        style={imgErrors[idx] ? { background: GRADIENT_FALLBACKS[idx % GRADIENT_FALLBACKS.length] } : {}}
                    >
                        {!imgErrors[idx] && (
                            <img
                                src={slide.image}
